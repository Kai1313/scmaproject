...
//FORMAT: 2024-11-11 11:11
var waktu_pakai = formResponse["Waktu"].split(" ");
var tanggal = waktu_pakai[0].substring(0, 4).replace(/[^\d.-]/g, '') + '-' + waktu_pakai[0].substring(3, 5).replace(/[^\d.-]/g, '') + '-' + waktu_pakai[0].substring(6, 8).replace(/[^\d.-]/g, '');
var waktu = waktu_pakai[1].split(":");
var waktu_pakai_izin = tanggal + ' ' + addZero(waktu[0]) + ':' + addZero(waktu[1]) + ':00';

var username_sisca = formResponse["Username SiSCA"];
var lokasi_dinas_luar = formResponse["Lokasi Dinas Luar"];
var keperluan = formResponse["Keperluan"];
if (typeof formResponse["Bukti Pendukung"] !== 'undefined') {
    var bukti_pendukung = formResponse["Bukti Pendukung"];
} else {
    var bukti_pendukung = "";
}
...
var data = {
    ...
    nama_perizinan: "",
    pengguna_perizinan: username_sisca,
    tanggal_perizinan: waktu_pakai_izin,
    lokasi_perizinan: lokasi_dinas_luar,
    keperluan_perizinan: convertEMOJI(keperluan),
    bukti_perizinan: convertEMOJI(bukti_pendukung.toString()),
    keterangan_perizinan: "",
    status_perizinan: '1',
    user_perizinan: 1,
    date_perizinan: sekarang_datetime,
...
var response = UrlFetchApp.fetch(protocol + "://" + host + "/" + directory + "/actions/perizinan_tambah.php?nc=" + (new Date()).getTime(), options);
