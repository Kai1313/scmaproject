...
class ReceivedFromWarehouseController extends Controller
{
    ...
    {
        if (checkUserSession($request, 'laporan_terima_dari_gudang', 'show') == false) {
        ...
        if ($request->ajax()) {
            ...
            $html = '';
            $html .= view('report_ops.receivedFromWarehouse.template', [
                'datas' => $data,
        ...
        return view('report_ops.receivedFromWarehouse.index', [
            "pageTitle" => "SCA OPS | Laporan Terima Dari Gudang | List",
            'arrayStatus' => $this->arrayStatus,
    ...
    function print(Request $request) {
        ...
        return view('report_ops.receivedFromWarehouse.print', [
            "pageTitle" => "SCA OPS | Laporan Terima Dari Gudang | Print",
            "datas" => $data,
            'arrayStatus' => $this->arrayStatus,
    ...
    public function getData($request)
    {
        ...
        switch ($type) {
            case 'Rekap':
            case 'Detail':
                $data = MoveBranch::where('id_jenis_transaksi', '24')->whereBetween('tanggal_pindah_barang', $date)
