...
class SendToWarehouseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $html = '';
            $html .= view('report_ops.sendToWarehouse.template', [
                'datas' => $data,
    ...
    public function getData($request)
    {
        ...
        switch ($type) {
            case 'Rekap':
            case 'Detail':
                $data = MoveBranch::where('id_jenis_transaksi', '23')->whereBetween('tanggal_pindah_barang', $date)
